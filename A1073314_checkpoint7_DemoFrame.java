...
public class A1073314_checkpoint7_DemoFrame extends JFrame {
    ...
    private ArrayList<A1073314_checkpoint7_Block> obstacle;
    ...
    private ArrayList<A1073314_checkpoint7_Block> sands;
    //Description : the object to query data.
    private A1073314_checkpoint7_QueryDB querydb;
    ...
    public A1073314_checkpoint7_DemoFrame(int FWidth, int FHeight,String mapID,int jfScaler, int milliseconds, int algorithm) throws HeadlessException {
        this.FWidth = FWidth;
        ...
        this.obstacle = new ArrayList<A1073314_checkpoint7_Block>();
        this.obstacleDataStructure = new ArrayList<Integer[]>();
        this.sands = new ArrayList<A1073314_checkpoint7_Block>();
        this.sandsDataStructure = new ArrayList<Integer[]>();
        this.algorithm = algorithm;
        this.querydb = new A1073314_checkpoint7_QueryDB(mapID);
        ...
        this.player = new A1073314_checkpoint7_Player("player",jfScaler,SwingConstants.CENTER);
        this.sandsDataStructure = querydb.getSands();
        this.sands = toGUIIdx(sandsDataStructure, SANDTYPE);
        A1073314_checkpoint7_DemoPanel panel = new A1073314_checkpoint7_DemoPanel(player, obstacle,jfScaler, sands);
        this.add(panel);
        ...
        player.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                int x = panel.getWidth()/2-panel.getCenterX();
                int y = panel.getHeight()/2-panel.getCenterY();
                panel.setCenterX((player.getMapX()+1)*panel.getGridLen());
        ...
        panel.addMouseListener(new MouseAdapter() {
            ...
            public void mouseReleased(MouseEvent e) {
                ...
                ReleasedX = e.getX();
                ReleasedY = e.getY();
                int locationX = ReleasedX - PressedX;
                int locationY = ReleasedY - PressedY;

                if(!player.getOnClick()){
                    panel.setCenterX(panel.getCenterX()-locationX);
                    panel.setCenterY(panel.getCenterY()-locationY);
                }
                panel.repaint();

                if(player.getOnClick()){
                    if(!player.getMoving()){
                        A1073314_checkpoint7_PlayerMovement playerMovement = new A1073314_checkpoint7_PlayerMovement(player, panel, ReleasedX, ReleasedY, milliseconds, algorithm, map);
    ...
    public ArrayList toGUIIdx(ArrayList<Integer[]> data, String blockType){
        ArrayList<A1073314_checkpoint7_Block> dataGui = new ArrayList<A1073314_checkpoint7_Block>();
        for(Integer[] x : data){
            if(blockType == "obstacle"){
                dataGui.add( new A1073314_checkpoint7_Block(x[1]-1, x[0]-1,blockType, obstacleImg.get(x[2]), 100000));
            }else if(blockType == "sand"){
                dataGui.add( new A1073314_checkpoint7_Block(x[1]-1, x[0]-1,blockType, new ImageIcon("./Resource/sand.jpg").getImage(), SANDWEIGHT));
            }
        }
        return dataGui;
    }
    ...
    public A1073314_checkpoint7_Block[][] createMap(int height,int width){
        A1073314_checkpoint7_Block[][] map = new A1073314_checkpoint7_Block[width][height];
        for(A1073314_checkpoint7_Block block : obstacle){
            map[block.getX()][block.getY()] = block;
        }
        for(A1073314_checkpoint7_Block block : sands){
            map[block.getX()][block.getY()] = block;
        }
        for(int x = 0; x<width; x++){
            for(int y = 0; y<height; y++){
                if(map[x][y] == null){
                    map[x][y] = new A1073314_checkpoint7_Block(x,y,"space",null, SAPCEWEIGHT);
                }
            }
        }
        return map;
